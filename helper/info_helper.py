...
class InfoHelper:
    ...
    def calc(self):
        ...
        if not os.path.exists(mapping.info_log):
            os.system(f"touch {mapping.info_log}")
        f = open("info.log", "r+")
        ...
        f.write(f"Monate bis Speicher voll: {round((estimated_cycles / 24) / 31, 1)} Monate \n")
        f.write(f"Jahre bis Speicher voll: {round(((estimated_cycles / 24) / 31) / 12, 1)} Jahre \n")
